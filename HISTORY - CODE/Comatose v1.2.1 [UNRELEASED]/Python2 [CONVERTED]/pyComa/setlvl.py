...
def cloc3(ctot, C):
    ...
    * - direction of creatures (start)
    '''
    for i in xrange(int(ctot / 2)):
        C.append(creature(283 + i * 2 * board.WIDTH, arrow.D))
    n = 0
    for i in xrange(int(ctot / 2), ctot):
...
def cloc4(C): # C must be empty []
    ...
    * - direction of creatures (start)
    ...
    if C[-1].loc == 0: #[0]
        ...
        k = 0
        for i in xrange(15):
            for j in xrange(board.WIDTH - 10):
...
def ploc5(ploc):
    ...
    * - location of pickups
    '''
    k = 0
    it = iter(xrange(1, board.HEIGHT, 2))
    for i in it:
        if (board.HEIGHT - 4 < i):
            it, None.next()
            k -= 6
        for j in xrange(1, board.WIDTH, 5):
...
def comatose():
    ...
    won = False
    if not lvlchange.gameover:
        sys.stdout.write(u"Fredrick wakes up!!")
        sys.stdout.flush()
        won = True
        getch()
        return False
    if not won:
        sys.stdout.write(u"To exit press enter. To retry press anything else.")
        sys.stdout.flush()
        c = u''
        c = getch()
        if c == u'\r' or c == u'\n':
