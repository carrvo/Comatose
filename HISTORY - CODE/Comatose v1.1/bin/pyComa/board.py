...
class board(object):
    ...
    def outside(self):
        for k in xrange(0, FIN):
            self.Boarder(k, symbol.E)
        for i in xrange(0, WIDTH):
            self.Boarder(i, symbol.BD)
        for i in xrange(FIN-WIDTH, FIN-1):
            self.Boarder(i, symbol.BD)
        for i in xrange(0, FIN, WIDTH):
            self.Boarder(i, symbol.BD)
        for i in xrange(WIDTH-1, FIN, WIDTH):
            self.Boarder(i, symbol.BD)
    ...
    def seeboard(self, bombs):
        ...
        for i in self.Boarder():
            tosee = u''.join((tosee, i.to()))
            if n == WIDTH:
                tosee = u''.join((tosee, u'\n'))
                n = 1
            else:
                n += 1
        tosee = u''.join((tosee, u"BOMBS: {}\n\n".format(bombs)))
...
class WALL(board):
    slow = None
    def __init__(self, cr, crhv, inn):
        super(WALL, self).__init__(cr, crhv, inn) #super(board, self)
        WALL.slow = False
