...
class BOSS(board.board):
    def __init__(self, cr, inn):
        super().__init__(None, False, inn) #super(board, self) #False for vertical
        #crhv? wBOSS? finBOSS?
        self.crBOSS = cr
    def outsideBOSS(self):
        for k in range(board.FIN):
            self.Boarder(k, symbol.E)
        for i in range(board.WIDTH):
            self.Boarder(i, symbol.BD)
        for i in range(board.FIN - 1, board.FIN - board.WIDTH, -1):
            self.Boarder(i, symbol.BD)
        for i in range(0, board.FIN, board.WIDTH):
            self.Boarder(i, symbol.BD)
        for i in range(board.WIDTH - 1, board.FIN + 1, board.WIDTH):
            self.Boarder(i, symbol.BD)
